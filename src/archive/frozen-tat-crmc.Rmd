...
# Turnaround Time Report: `r lubridate::year(params$dt_start)` Q`r lubridate::quarter(params$dt_start)` 2020 Q1--Q4
...
ta_time %>%
  ...
  tab_header(title = md("First Quarter Turnaround Time Statistics"),
             subtitle = "") %>%
...
```{r Q2 table, message = FALSE, warning = FALSE}
# Quarter 2 -----
ta_time %>%
  mutate(
    Create = date(Create),
  ...
  filter(`original release` >= "2020-04-01" & 
           `original release` <= "2020-06-30") %>%
  group_by(PATHOLOGIST) %>%
  summarize(
    frozen = sum(!is.na(tat_frozen)),
    `< 20 min` = sum(tat_frozen <= 20, na.rm = TRUE),
  ...
  gt(rowname_col = "PATHOLOGIST", auto_align = TRUE) %>%
  tab_header(title = md("Second Quarter Turnaround Time Statistics"),
             subtitle = "") %>%
  tab_spanner(label = md("**Frozen (under 20 min)**"),
              vars(frozen,
                   `< 20 min`,
...
```{r Q3 table, message = FALSE, warning = FALSE}
# Quarter 3 -----
ta_time %>%
  mutate(
    Create = date(Create),
  ...
  filter(`original release` >= "2020-07-01" & 
           `original release` <= "2020-09-30") %>%
  group_by(PATHOLOGIST) %>%
  summarize(
    frozen = sum(!is.na(tat_frozen)),
    `< 20 min` = sum(tat_frozen <= 20, na.rm = TRUE),
  ...
  gt(rowname_col = "PATHOLOGIST", auto_align = TRUE) %>%
  tab_header(title = md("Third Quarter Turnaround Time Statistics"),
             subtitle = "") %>%
  tab_spanner(label = md("**Frozen (under 20 min)**"),
              vars(frozen,
                   `< 20 min`,
...
```{r Q4 table, message = FALSE, warning = FALSE}
# Quarter 4 -----
ta_time %>%
  mutate(
    Create = date(Create),
  ...
  filter(`original release` >= "2020-10-01" & 
           `original release` <= "2020-12-31") %>%
  group_by(PATHOLOGIST) %>%
  summarize(
    frozen = sum(!is.na(tat_frozen)),
    `< 20 min` = sum(tat_frozen <= 20, na.rm = TRUE),
  ...
  gt(rowname_col = "PATHOLOGIST", auto_align = TRUE) %>%
  tab_header(title = md("Fourth Quarter Turnaround Time Statistics"),
             subtitle = "") %>%
  tab_spanner(label = md("**Frozen (under 20 min)**"),
              vars(frozen,
                   `< 20 min`,
